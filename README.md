...
4. `/mybots` → 选择你的 bot → Bot Settings → 开启 **Bot-to-Bot Communication Mode**(关键!开启后 bot 才能收到群内其他 bot 的消息,从而识别并删除 guest bot 召唤投放的广告。不开启则 guest bot 广告对本 bot 完全不可见)
...
| `/unbind` | 解除当前群的绑定,清空缓存、群级 bot 白名单与违规计数 |
| `/status` | 查看当前绑定、缓存命中、近 1h 删除计数、最近错误、群级 bot 白名单 |
| `/allowbot @bot` | 把指定 bot 加入本群 bot 白名单,其消息与 guest 应答不再被删除(也可回复某 bot 的消息执行) |
| `/disallowbot @bot` | 从本群 bot 白名单移除指定 bot |
...
| `allowlist` | `BOT_ALLOWLIST` | `[]` | 全局白名单 bot 的 user_id 列表,逗号分隔(环境变量形式);各群另可用 `/allowbot` 维护群级白名单 |
| `allow_anonymous_admin` | `BOT_ALLOW_ANONYMOUS_ADMIN` | `true` | 是否放行群匿名管理员 |
| `guest_mute_threshold` | `BOT_GUEST_MUTE_THRESHOLD` | `2` | 召唤未授权 guest bot 累计达此次数 → 禁言召唤者(必须 ≥ 2,保证首次不罚) |
| `guest_mute_duration` | `BOT_GUEST_MUTE_DURATION` | `24h` | 禁言时长 |
| `guest_ban_threshold` | `BOT_GUEST_BAN_THRESHOLD` | `4` | 召唤违规累计达此次数 → 封禁召唤者(必须 > 禁言阈值) |
